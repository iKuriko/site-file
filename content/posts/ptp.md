...
tags:
  - Services
description: PTP，Precision Time Protocol（精确时间协议）是在计算机网络中实现亚微秒级甚至纳秒级高精度时钟同步的协议，协议编号为IEEE 1588
---



在Linux中使用 **linuxptp** 套件（`ptp4l` + `phc2sys`） 实现高精度时间同步，并尽可能精确地测量同步后的残余偏差，评估同步精度和稳定性 
