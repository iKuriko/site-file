...
| -D       | 列出所有可抓取的网口                                         |
| -i       | 指定抓取的网口，any表示所有网口                              |
| -n       | 不进行主机名和端口号的反向解析，直接显示 IP 地址和数字端口，加快抓包速度并避免 DNS 干扰 |
...
| -v       | 详细使用 (-v) 或 (-vv) 或 (-vvv) 会增加输出中显示更详细信息，通常会显示更多协议特定的信息 |
| -A       | 输出中包含抓包的 `ascii` 字符串                              |
| -X       | 同时显示十六进制输出和 `ascii` 字符串，详细使用 (-XX)  包括链路层头部（如以太网帧头），比 `-X`（仅打印 IP 层以上数据）更底层 |
